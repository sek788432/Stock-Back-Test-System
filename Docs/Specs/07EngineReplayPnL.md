...
| Status | Scope |
| --- | --- |
| **Implemented** | CSV-backed bar loading; forward/back bar stepping; replay clock controls; candlestick/volume presentation; an initial-capital display; legacy JSON replay-summary persistence/comparison. This is **bar-only replay**, not a trading backtest. |
| **Planned** | `MarketSlice`, strategies, indicators, broker simulation, orders/fills, portfolio/P&L, short margin, corporate actions, metrics, immutable snapshots, and SQLite `.bteresult`. |
...
Legacy JSON snapshots are implemented current behavior, not the target result contract. They must not be described as complete backtest results.
