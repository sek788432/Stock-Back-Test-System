...
**Tech Stack:** Markdown Agent Skills, YAML UI metadata, Codex skill validator, Git, GitHub CLI
...
- [ ] **Step 1: Run three independent scenarios without supplying the proposed skill**
...
- [ ] **Step 2: Record observable baseline failures**
...
- [ ] **Step 3: Convert failures into skill requirements**
...
- [ ] **Step 1: Rewrite the skill discovery metadata and overview**
...
- [ ] **Step 2: Define the preflight and slicing contract**
...
- [ ] **Step 3: Define the per-slice execution loop**

Require test-first work at a pre-agreed seam, focused tests and formatting during iteration, broader affected checks before accepting the slice, and no work on later slices until the current slice is green.

- [ ] **Step 4: Define checkpoints, failure handling, and final verification**
...
- [ ] **Step 5: Add quick reference, shortcut counters, and common mistakes**
...
- [ ] **Step 6: Update UI metadata and the project skill catalog**
...
- [ ] **Step 7: Review the documentation diff before testing**
...
git commit -m "feat(skills): implement plans in CI-green slices"
```
...
- [ ] **Step 1: Re-run the three pressure scenarios with the revised skill**
...
- [ ] **Step 2: Close observed loopholes and re-test**
...
- [ ] **Step 3: Validate the skill package and YAML**

```bash
python3 /Users/shaohsua/.codex/skills/.system/skill-creator/scripts/quick_validate.py .agents/skills/implement
ruby -e 'require "yaml"; YAML.load_file(".agents/skills/implement/agents/openai.yaml"); puts "openai.yaml: valid"'
```
...
- [ ] **Step 5: Commit any verification-driven refinements**

```bash
git add .agents/skills/implement/SKILL.md .agents/skills/implement/agents/openai.yaml .agents/skills/README.md
git commit -m "docs(skills): harden incremental implementation gates"
```

Skip this commit when pressure testing requires no refinement.
